/**
 * HTML5 LightBox - jQuery Image and Video LightBox Plugin Copyright 2014 Magic
 * Hills Pty Ltd All Rights Reserved Website: http://html5box.com Version 6.4
 */
(function() {
...
function loadHtml5LightBox(jsFolder) {
	(function($) {
		$.fn.html5lightbox = function(options) {
					...
					.extend(
							{
								...
								fullscreenmodeonsmallscreen : true,
								fullscreennomarginonsmallscreen : true,
			...
			inst.options.types = [ "IMAGE", "FLASH", "VIDEO", "YOUTUBE",
					"VIMEO", "PDF", "MP3", "WEB", "FLV", "DAILYMOTION", "DIV",
					"WISTIA", "IFRAMEVIDEO" ];
			...
			inst.options.html5VideoSupported = !!document
			...
			if ((inst.options.isIE6789 || inst.options.isIE10 || inst.options.isIE11)
			...
			if (inst.options.initsocial)
				$("head").append(
						'<link rel="stylesheet" href="' + inst.options.jsfolder
			...
			inst.createMarkup = function() {
				...
				if (inst.options.titlecss.length > 1) {
					if (inst.options.titlecss.charAt(0) == "{")
						inst.options.titlecss = inst.options.titlecss
								.substring(1);
					if (inst.options.titlecss
							.charAt(inst.options.titlecss.length - 1) == "}")
						inst.options.titlecss = inst.options.titlecss
								.substring(0, inst.options.titlecss.length - 1)
				}
				inst.options.descriptioncss = $
						.trim(inst.options.descriptioncss);
				...
				if (inst.options.titlestyle == "right") {
					styleCss += "#html5box-html5-lightbox #html5-elem-wrap {width:"
				...
				inst.$lightbox = $("<div id='html5box-html5-lightbox' style='display:none;top:0px;left:0px;width:100%;height:100%;z-index:9999998;text-align:center;'>"
						...
						+ "px;'></div>"
						+ "</div>" + "</div>");
				inst.options.positionFixed = inst.options.supportCSSPositionFixed
						&& inst.options.responsive
						&& !inst.options.iequirksmode;
				inst.$lightbox.css({
					position : inst.options.positionFixed ? "fixed"
							: "absolute"
				});
				...
				if (inst.options.titlestyle == "right"
					...
					inst.$elemData.css({
						...
						"padding" : "0 0 " + inst.options.bordersize + "px"
								+ " 0",
						"background-color" : inst.options.bgcolor,
						"text-align" : "left"
					});
				...
				if (inst.options.showsocial) {
					...
					var socialBtnCSS = (inst.options.socialdirection == "horizontal" ? "display:inline-block;"
							: "display:block;")
							+ "margin:4px;";
					...
					$(".html5-social-btn", inst.$lightbox)
							.click(
									function() {
										...
										if (inst.currentElem[ELEM_TYPE] == 0)
											shareMedia = inst
													.absoluteUrl(inst.currentElem[ELEM_HREF]);
										else if (inst.currentElem[ELEM_TYPE] == 3)
											shareMedia = "https://img.youtube.com/vi/"
													+ inst
															.getYoutubeId(inst.currentElem[ELEM_HREF])
													+ "/0.jpg";
										else {
											var lightboxLink = $('.html5lightbox[href="'
											...
											if (lightboxLink.length > 0)
												if (lightboxLink
														.data("shareimage")
														&& lightboxLink
																.data("shareimage").length > 0)
													shareMedia = inst
															.absoluteUrl(lightboxLink
																	.data("shareimage"));
													...
													if (lightboxImg.length > 0)
														shareMedia = inst
																.absoluteUrl(lightboxImg
																		.attr("src"))
												}
										}
										var isVideo = inst.currentElem[ELEM_TYPE] == 2
			...
			inst.calcBoxPosition = function(initW, initH) {
				...
				var winH = window.innerHeight ? window.innerHeight : $(window)
						.height();
				var boxT = Math.round((winH - inst.options.navheight) / 2
						- boxH / 2);
				...
				if (inst.options.insideiframe && window.self != window.top)
					if (parent.window.jQuery
							&& parent.window
									.jQuery("#" + inst.options.iframeid).length) {
			...
			inst.clickHandler = function() {
				...
				if (!inst.options.positionFixed)
					inst.$lightboxBox.css("margin-top", $(window).scrollTop()
							+ inst.options.bordertopmargin);
				if (inst.options.titlestyle == "left"
							...
							$(".html5-nav-list")
									.append(
											"<div class='html5-nav-thumb' data-arrayindex='"
													...
													+ ";'></div>");
							this.loadThumbnail(
									inst.elemArray[i][ELEM_THUMBNAIL], index);
							index++
						}
				$(".html5-nav-thumb")
						.hover(
						...
						inst
								.resizeLightbox(
										...
										function() {
											...
											inst.$image.show();
											inst.$image
													.html("<div id='html5-image-container' style='display:block;position:relative;width:100%;height:100%;"
															+ (inst.options.imagekeepratio ? ""
																	: "overflow-x:auto;overflow-y:scroll;-ms-overflow-x:auto;-ms-overflow-y:scroll;")
															+ "'><img src='"
															+ elem[ELEM_HREF]
															+ "' width='100%' height='"
															+ (inst.options.imagekeepratio ? "100%"
																	: "auto")
															+ "' />"
															+ timercode
															+ "</div>");
									...
									inst.$image
											.html(
													"<div id='html5lightbox-div' style='display:block;width:100%;height:"
															+ (inst.options.autoresizecontent ? "auto"
																	: "100%")
															+ ";"
			...
			inst.resizeWindow = function() {
				...
				if (inst.options.autoresizecontent
					...
					if (inst.currentElem[ELEM_TYPE] == 7
							...
							&& $("#html5lightbox-web-iframe", inst.$lightbox)
					...
					} else if (inst.currentElem[ELEM_TYPE] == 10
							&& $("#html5lightbox-div", inst.$lightbox).length > 0) {
						var divH = $("#html5lightbox-div", inst.$lightbox)
								.height();
						if (elemH > divH) {
							elemH = divH;
							resizeHeight = true
					...
					if (resizeHeight) {
						...
						if (!inst.options.positionFixed)
							inst.$lightboxBox.css("margin-top", $(window)
									.scrollTop()
									+ inst.options.bordertopmargin);
						if (inst.options.titlestyle == "left"
								|| inst.options.titlestyle == "right")
				...
				$container
						.html("<div style='display:block;width:100%;height:100%;position:relative;'><video id='html5-lightbox-video' width='100%' height='100%'"
								+ (inst.options.html5videoposter
										&& inst.options.html5videoposter.length > 0 ? "poster='"
										+ inst.options.html5videoposter + "'"
										: "")
								+ (autoplay ? " autoplay" : "")
								+ (loopvideo ? " loop" : "")
			...
			inst.addItem = function(href, title, group, width, height, webm,
					ogg, thumbnail, description, mediatype) {
				type = mediatype && mediatype >= 0 ? mediatype : inst
						.checkType(href);
			...
			inst.each(function() {
				...
				if (self.data("autoopen")) {
					setTimeout(function() {
						self.click()
					}, self.data("autoopendelay") ? self.data("autoopendelay")
							: 0);
					return false
				}
			});
	...
	(function($) {
		$.fn.lightboxHTML5VideoControls = function(skinFolder, parentInst,
			...
			var $videoSeekerHandler = $(".html5boxSeekerHandler",
					$videoControls);
			...
			if ($videoObj.get(0).volume === volume / 2 + 0.1) {
				...
				var $videoVolumeBarBg = $(".html5boxVolumeBarBg",
						$videoControls);
